...
<template>
  <div class="flex flex-col w-72 h-screen min-h-screen relative">
    ...
    <div class="bg-[#201c2c] h-screen w-full overflow-y-auto sidebar-custom py-4">
      <div class="flex flex-col h-auto min-h-[100%] w-64 mx-auto relative border-4 border-dashed " :class="[over ? 'bg-sky-500 bg-opacity-40 rounded border-sky-400' : 'border-transparent']" @drop="onDrop($event, event.name)" @dragenter.prevent @dragover.prevent @dragenter="dragEnter" @dragleave="dragLeave">
